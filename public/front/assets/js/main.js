...
// add  counter js
            ...
            function animateCounter(element, target, duration, finalLabel) {
              let start = 0;
            ...
            document.addEventListener("DOMContentLoaded", () => {
              ...
              function handleScroll() {
                counters.forEach((counter) => {
                  ...
                  if (rect.top < window.innerHeight && !started.has(counter)) {
                    started.add(counter);
                    const target = parseInt(counter.getAttribute('data-target'));
                    const finalLabel = counter.getAttribute('data-label');
                    animateCounter(counter, target, 2000, finalLabel);
                  }
                });
              }
            ...
            let items = document.querySelectorAll(".carousel-item-custom");
            const visibleItems = 5;
            const itemWidth = items[0].offsetWidth + 20; // width + margin
            let position = 0;
            let currentIndex = visibleItems;
            
            // Clone first and last few items
            function setupCarouselLoop() {
              ...
              for (let i = 0; i < visibleItems; i++) {
                ...
                lastClones.unshift(cloneLast);
              }
            
              firstClones.forEach(clone => track.appendChild(clone));
              lastClones.forEach(clone => track.prepend(clone));
            
              // Re-query all items after cloning
              items = document.querySelectorAll(".carousel-item-custom");
            
              // Set initial scroll position to "real" first item
            ...
            function slideLeft() {
              ...
              if (currentIndex <= 0) {
                setTimeout(() => {
                  track.style.transition = "none";
                  currentIndex = items.length - visibleItems * 2;
                  position = -itemWidth * currentIndex;
                  track.style.transform = `translateX(${position}px)`;
                  setTimeout(() => {
                    track.style.transition = "transform 0.5s ease";
            ...
            window.addEventListener("load", setupCarouselLoop);
            // 
